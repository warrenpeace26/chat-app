    ...
    text-white ${selectedUser ? "max-md-hidden" : ''}`}>
      <div className='pb-5'>
        ...
        <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5' >
          ...
          text-white text-xd placeholder-[#c8c8c8] flex-1' placeholder='search user... ' />
        </div>
    ...
    <div className='flex flex-col'>
      ...
      {filteredUsers.map((user, index)=>(
        ...
        cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
          ...
          flex justify-center items-center rounded-full bg-voilet-500/50'>{unseenMessages[user._id]}</p> }
        </div>
      ) )}        
    </div>  
